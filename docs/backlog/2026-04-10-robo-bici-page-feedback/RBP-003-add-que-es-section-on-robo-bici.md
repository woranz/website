# RBP-003 - Add que es section on robo bici
...
- Entregable estructural: insertar la sección `Qué es` en robo bici usando la fuente de contenido ya mapeada en el rollout transversal.
- Visual: si
- Owner visual: Claude
...
La página de `robo-bici` todavía no expone una sección `Qué es`, aunque el template ya la soporta y el texto fuente ya existe en el inventario editorial extraído del documento de contenidos.
...
- `docs/backlog/2026-04-10-que-es-rollout/QES-001-source-inventory.md` ya mapea el texto aprobado para `personas/robo-bici`.
...
- Agregar la sección `Qué es` a robo bici.
...
- `/personas/coberturas/robo-bici` muestra la sección `Qué es` con el texto aprobado.
...
- Verificación estática de la fuente de verdad para `personas/robo-bici`.
