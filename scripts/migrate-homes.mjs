/**
 * Migra las 3 páginas home a Sanity.
 *
 * Antes de correr:
 *   1. npx sanity login
 *   2. node scripts/migrate-homes.mjs
 */
...
async function migrate() {
  console.log('Migrando 3 homes a Sanity...\n')

  for (const home of homes) {
    try {
      const result = await client.createOrReplace(home)
      console.log(`✅ ${home.segmento} → ${result._id}`)
    } catch (err) {
      console.error(`❌ ${home.segmento}:`, err.message)
    }
  }

  console.log('\n¡Listo! Abrí /studio para verificar.')
}
